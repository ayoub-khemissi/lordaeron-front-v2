...
const raidProgression: RaidRow[] = [
  ...
  { type: "separator", label: "Wrath of the Lich King" },
  {
    type: "raid",
    tier: "Tier 7",
    raids: "Naxxramas / Malygos / Sartharion",
    ilvl: "200-213",
  },
  { type: "raid", tier: "Tier 8", raids: "Ulduar", ilvl: "219-239" },
  {
    type: "raid",
    tier: "Tier 9",
    raids: "Trial of the Crusader / Onyxia 80",
    ilvl: "232-258",
  },
  {
    type: "raid",
    tier: "Tier 10",
    raids: "Icecrown Citadel (ICC)",
    ilvl: "251-277",
  },
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl group">
      <Image
        ...
        loading="lazy"
        sizes="(max-width: 768px) 100vw, 900px"
        src="/img/Wrath Classic Fall of the Lich King Launch Screenshots/WoW_WrathClassic_Fall_of_the_Lich_King_(3)_png_jpgcopy.jpg"
      />
      <div className="absolute inset-0 bg-gradient-to-b from-wow-darker/60 to-wow-darker/90" />
          ...
          <table className="w-full min-w-[500px]">
            <thead>
              <tr className="bg-wow-darker/60 border-b border-wow-gold/10">
                <th className="text-wow-gold text-xs uppercase tracking-wider text-left px-4 py-3">
                  {t("tier")}
                </th>
                <th className="text-wow-gold text-xs uppercase tracking-wider text-left px-4 py-3">
                  {t("raids")}
            ...
            <tbody>
              {raidProgression.map((row, i) =>
                row.type === "separator" ? (
                  <tr key={i}>
                    <td
                      className="px-4 py-3 border-b border-wow-gold/10"
                      colSpan={3}
                    >
                  ...
                  >
                    <td className="px-4 py-3">
                      <span
                        className={`font-bold whitespace-nowrap ${row.tier === "Final" ? "text-wow-gold drop-shadow-[0_0_6px_rgba(199,156,62,0.5)]" : row.tier.includes("Transition") ? "text-gray-400 italic" : "text-wow-blue"}`}
                      >
                        {row.tier}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3">
                      <span
                        className={`text-sm font-mono ${row.ilvl ? "text-wow-gold-light" : "text-default-600"}`}
                      >
                        {row.ilvl || "-"}
                      </span>
                    </td>
