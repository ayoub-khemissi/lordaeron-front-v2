...
export function HighlightsCarousel({ items, sets = [], onItemClick, onSetClick }: HighlightsCarouselProps) {
  ...
  return (
    <section className="mb-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
        {sets.map((set) => (
          <SetCard key={`set-${set.id}`} set={set} onClick={() => onSetClick?.(set)} />
        ))}
        {items.map((item) => (
          <ItemCard key={`item-${item.id}`} item={item} onClick={() => onItemClick(item)} />
        ))}
      </div>
    </section>
  );
