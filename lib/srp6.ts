...
const g = 7n;
const N =
  0x894b645e89e1535bbdad5b8b290650530801b18ebfbf5e8fab3c82872a3e9bb7n;
...
export function calculateVerifier(
  ...
  const innerHash = new Uint8Array(crypto.createHash("sha1").update(credentials).digest());

  const combined = new Uint8Array(salt.length + innerHash.length);
  combined.set(salt);
  combined.set(innerHash, salt.length);

  const outerHash = new Uint8Array(crypto.createHash("sha1").update(combined).digest());
