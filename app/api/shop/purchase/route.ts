...
import { createPurchase, updatePurchaseStatus } from "@/lib/queries/shop-purchases";
import { getActiveBan } from "@/lib/queries/shop-bans";
import { getCharactersByAccount, getCharacterByExactName } from "@/lib/queries/characters";
...
async function handleSetPurchase(
  ...
  if (body.is_gift) {
    if (!body.gift_to_character_name) {
      return NextResponse.json({ error: "giftCharacterRequired" }, { status: 400 });
    }

    const recipient = await getCharacterByExactName(body.gift_to_character_name);
    if (!recipient) {
      return NextResponse.json({ error: "giftRecipientNotFound" }, { status: 400 });
    }

    if (set.class_ids && !set.class_ids.includes(recipient.class)) {
      return NextResponse.json({ error: "giftRecipientClassRestricted" }, { status: 400 });
    }

    const giftFactionError = checkFaction(set.faction, recipient.race);
    if (giftFactionError) {
      return NextResponse.json({ error: "giftRecipientFactionRestricted" }, { status: 400 });
    }

    if (set.min_level > 0 && recipient.level < set.min_level) {
      return NextResponse.json({ error: "giftRecipientLevelRestricted" }, { status: 400 });
    }
  ...
  if (set.items.length > 0) {
    const subject = body.is_gift
      ...
      : "Lordaeron Shop";
    const mailBody = body.is_gift && body.gift_message
      ? body.gift_message
  ...
  try {
    ...
    if (body.is_gift) {
      ...
      if (item.category === "services") {
        return NextResponse.json({ error: "giftNotAvailableForServices" }, { status: 400 });
      }
      ...
      if (item.race_ids && !item.race_ids.includes(recipient.race)) {
        return NextResponse.json({ error: "giftRecipientRaceRestricted" }, { status: 400 });
      }
